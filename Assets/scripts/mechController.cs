...
/// <summary>
/// Mech AI (mechController) based on Enemy2Controller, with:
...
[Tooltip("If the current combat ally gets farther than this for keepChasingTime, we drop them and fall back.")]
    ...
    [Tooltip("Small offset forward from each fire point to avoid immediately raycasting into our own collider.")]
    ...
    [Tooltip("If true, the firing cone uses each fire point forward (aim) instead of transform.forward (body).")]
...
[Header("Muzzle Flash (Simple Toggle)")]
    [Tooltip("Primary muzzle flash object for the left / primary gun.")]
    public GameObject muzzleFlashObject;
    [Tooltip("Optional second muzzle flash object for the right / secondary gun.")]
    public GameObject secondaryMuzzleFlashObject;
    [Range(0.005f, 0.2f)]
    [Tooltip("How long the muzzle flash stays visible per shot (seconds). Typical 0.03 - 0.08")]
    public float muzzleFlashDuration = 0.05f;
    [Tooltip("If true, we snap the muzzle flash object(s) to the assigned fire point(s) when firing.")]
    public bool muzzleFlashFollowFirePoint = true;
    [Tooltip("Optional local offset from fire point when snapping.")]
    ...
    private Coroutine _secondaryMuzzleFlashRoutine;
...
public void SetWaterSlow(bool inWater, float speedMultiplier)
    ...
    private bool HasAnyFirePoint()
    {
        return firePointLeft != null || firePointRight != null;
    }


    private bool TryFireFromPoint(Transform currentFirePoint, Transform target, Vector3 aimPos)
    {
        if (currentFirePoint == null || bullet == null || target == null) return false;
        if (TargetIsDeadOrInvalid(target)) return false;

        currentFirePoint.LookAt(aimPos);

        Vector3 aimFrom = currentFirePoint.position;
        Vector3 aimFwd = (useFirePointForAimGate ? currentFirePoint.forward : transform.forward);
        ...
        {
            Vector3 losFrom = currentFirePoint.position + currentFirePoint.forward * Mathf.Max(0f, lineOfSightFirePointForwardOffset);
        ...
        GameObject spawned = Instantiate(bullet, currentFirePoint.position, currentFirePoint.rotation);
...
void HandleShooting(Transform target)
    {
        if (_isDead) return;
        if (bullet == null || !HasAnyFirePoint() || target == null) return;
        ...
        // Fire first burst volley immediately
        FireOnceAt(target);
        _burstRemaining--;
    ...
    private void FireOnceAt(Transform target)
    {
        ...
        bool firedLeft = TryFireFromPoint(firePointLeft, target, aimPos);
        bool firedRight = TryFireFromPoint(firePointRight, target, aimPos);
        ...
        if (debugMuzzleFlash)
            Debug.Log($"[mechController] Shot fired -> triggering muzzle flash on {name}", this);
    ...
    private void DisableMuzzleFlashImmediate()
    {
        ...
        if (_secondaryMuzzleFlashRoutine != null)
        {
            StopCoroutine(_secondaryMuzzleFlashRoutine);
            _secondaryMuzzleFlashRoutine = null;
        }

        if (muzzleFlashObject != null && muzzleFlashObject.activeSelf)
        {
            muzzleFlashObject.SetActive(false);
            if (debugMuzzleFlash) Debug.Log($"[mechController] MuzzleFlash FORCE OFF: {muzzleFlashObject.name}", muzzleFlashObject);
        }

        if (secondaryMuzzleFlashObject != null && secondaryMuzzleFlashObject.activeSelf)
        {
            secondaryMuzzleFlashObject.SetActive(false);
            if (debugMuzzleFlash) Debug.Log($"[mechController] Secondary MuzzleFlash FORCE OFF: {secondaryMuzzleFlashObject.name}", secondaryMuzzleFlashObject);
        }
    }

    private void TriggerMuzzleFlashSimple()
    {
        TriggerMuzzleFlashForObject(muzzleFlashObject, firePointLeft, true);
        TriggerMuzzleFlashForObject(secondaryMuzzleFlashObject, firePointRight, false);
    }

    private void TriggerMuzzleFlashForObject(GameObject flashObject, Transform currentFirePoint, bool primary)
    {
        if (flashObject == null) return;

        if (muzzleFlashFollowFirePoint && currentFirePoint != null)
        {
            flashObject.transform.position = currentFirePoint.TransformPoint(muzzleFlashLocalOffset);
            flashObject.transform.rotation = currentFirePoint.rotation;
        }

        if (flashObject.activeSelf) flashObject.SetActive(false);
        if (debugMuzzleFlash) Debug.Log($"[mechController] MuzzleFlash OFF: {flashObject.name}", flashObject);

        flashObject.SetActive(true);
        if (debugMuzzleFlash) Debug.Log($"[mechController] MuzzleFlash ON: {flashObject.name}", flashObject);
        ...
        if (primary)
        {
            ...
            _muzzleFlashRoutine = StartCoroutine(DisableMuzzleFlashAfterDelay(flashObject, true));
        }
        else
        {
            if (_secondaryMuzzleFlashRoutine != null)
            ...
            _secondaryMuzzleFlashRoutine = StartCoroutine(DisableMuzzleFlashAfterDelay(flashObject, false));
        }
    }

    private IEnumerator DisableMuzzleFlashAfterDelay(GameObject flashObject, bool primary)
    {
        yield return new WaitForSeconds(Mathf.Max(0.005f, muzzleFlashDuration));
        if (flashObject != null)
            flashObject.SetActive(false);
        if (debugMuzzleFlash && flashObject != null) Debug.Log($"[mechController] MuzzleFlash OFF: {flashObject.name}", flashObject);

        if (primary) _muzzleFlashRoutine = null;
        else _secondaryMuzzleFlashRoutine = null;
    }
