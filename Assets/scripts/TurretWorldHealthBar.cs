// TurretWorldHealthBar.cs
// World-space overhead health bar for turrets, matching EnemyWorldHealthBar behavior.
// Bind this to a child HealthBar object above the turret.

using UnityEngine;

[DisallowMultipleComponent]
public class TurretWorldHealthBar : MonoBehaviour
{
    [Header("References")]
    ...
    [Header("Binding")]
    [Tooltip("If empty, auto-finds TurretHealthController in parents/children of this object.")]
    [SerializeField] private TurretHealthController turretHealth;
    ...
    private void TryAutoWire()
    {
        ...
        {
            turretHealth = GetComponentInParent<TurretHealthController>();
            if (turretHealth == null) turretHealth = GetComponentInChildren<TurretHealthController>(true);
        }
    }
    ...
    private void BindIfNeeded()
    {
        ...
        if (turretHealth == null) TryAutoWire();
        if (turretHealth == null) return;

        turretHealth.OnHealth01Changed -= OnHealth01Changed;
        turretHealth.OnHealth01Changed += OnHealth01Changed;
        _isBound = true;
    }
    ...
    {
        if (turretHealth != null)
            OnHealth01Changed(turretHealth.Health01());
    }
