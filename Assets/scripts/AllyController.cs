...
public class AllyController : MonoBehaviour
{
    ...
    [Range(0f, 2f)] public float footstepVolume = 0.7f;
    [Tooltip("Seconds between footstep sounds while the ally is moving.")]
    ...
    [Range(0.5f, 1.5f)] public float minFootstepPitch = 0.95f;
    [Range(0.5f, 1.5f)] public float maxFootstepPitch = 1.05f;
...
if (combatStats != null)
    ...
    private void ResolveFootstepAudioSourceIfNeeded()
    {
        ...
        // Fallback: reuse the same source if this object only has one.
    ...
    private void HandleFootstepAudio()
    {
        ...
        bool isActuallyMoving = !agent.isStopped && Mathf.Max(agent.velocity.magnitude, agent.desiredVelocity.magnitude) > Mathf.Max(0.01f, footstepMinMoveSpeed);
        if (!isActuallyMoving)
        {
            _footstepTimer = 0f;
            return;
